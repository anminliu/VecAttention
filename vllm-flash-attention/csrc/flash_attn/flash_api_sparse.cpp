...
void set_params_fprop_sparse(Flash_fwd_params_sparse &params,
                            ...
                            const at::Tensor k,
                            const at::Tensor v,
                            const size_t block_size, // block size for the sparse attention
    ...
    params.BLOCK_SIZE_Q = block_size;
    // // params.NUM_ROWS should be equal to cdiv(seqlen_q, BLOCK_M), and BLOCK_M has to be 64 for now.
    // constexpr int BLOCK_M = 64;
    int expected_num_rows = (seqlen_q + params.BLOCK_SIZE_Q - 1) / params.BLOCK_SIZE_Q;
    TORCH_CHECK(params.NUM_ROWS == expected_num_rows);
    TORCH_CHECK(params.BLOCK_SIZE_Q == 64 || params.BLOCK_SIZE_Q == 128,
                "BLOCK_SIZE_Q must be 64 or 128 when BLOCK_SIZE_K=64 after tuning");
...
mha_fwd_sparse(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
               ...
               const int64_t block_size, // block size for the sparse attention
               const at::Tensor &block_count,
               const at::Tensor &block_offset,
               const at::Tensor &column_count,
               const at::Tensor &column_index,
    ...
    set_params_fprop_sparse(params,
                            ...
                            block_size, block_count, block_offset,
