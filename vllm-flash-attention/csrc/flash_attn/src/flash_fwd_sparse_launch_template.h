...
void run_mha_fwd_sparse_hdim32(Flash_fwd_params_sparse &params, cudaStream_t stream) {
    ...
    BLOCK_SIZE_SWITCH(params.BLOCK_SIZE_Q, [&]{
        DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
            run_flash_sparse_fwd<Flash_fwd_kernel_traits<Headdim, kBlockSize, 64, 4, false, false, T>, Is_dropout, Is_causal>(params, stream);
        });
    });
}
