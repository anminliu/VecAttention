...
from spattn.src.utils import causal_mask_in_uneqal_block
...
CONFIG_PATH = os.path.join(_KERNEL_DIR, "cache", f"{env_prefix}_vecattention_kernels_best_eff_configs.pkl")
...
def fuse_qk_softmax_minp_wo_causal(
    ...
    if USE_TRITON_AUTOTUNE:
        ...
        kernel_autotune_args = load_best_eff_configs((q_pooling_size, k_local_size, group_k_block, head_dim, causal))
    ...
    avg_q_chunk = torch.cat(
        [
            ...
            torch.zeros(
                ...
                dtype=avg_q_chunk.dtype, device=avg_q_chunk.device
            ),
        ],
        dim=2
    )
    ...
    k = torch.cat(
        [
            ...
            torch.full(
                ...
                torch.nan, dtype=k.dtype, device=k.device
            ),
        ],
        dim=2
    )
    ...
    column_index = torch.zeros(
        (batch_size, num_heads, padded_q_num_blocks, padded_k_len), dtype=torch.int32, device=avg_q_chunk.device
    )
    grid = lambda META: (
    ...
    if isinstance(gap, float):
        kernel_args = [
            avg_q_chunk, k, sm_scale,
            column_count, column_index,
            avg_q_chunk.stride(0), avg_q_chunk.stride(1), avg_q_chunk.stride(2),
            k.stride(0), k.stride(1), k.stride(2),
            column_count.stride(0), column_count.stride(1),
            column_index.stride(0), column_index.stride(1), column_index.stride(2),
            avg_q_chunk_offset, avg_q_chunk_real_length, k_len,
            num_heads, head_dim,
            gap, causal,
            q_pooling_size,
            k_local_size,
    ...
    column_index = column_index[:, :, :q_num_blocks, :max(min(k_len, max_column_count), 1)].contiguous()
...
def _causal_fuse_qk_cutoff_wo_causal_kernel(
    ...
    stride_ccb, stride_cch,
    stride_cib, stride_cih, stride_cin,
    q_chunk_offset, q_length, k_length,
    NUM_HEADS: tl.constexpr, HEAD_DIM: tl.constexpr,
    gap: tl.constexpr, CAUSAL: tl.constexpr,
    ...
    k_offset_start = kblock_id * BLOCK_SIZE_K if not wo_init else max(q_pooling_size, kblock_id * BLOCK_SIZE_K)
    max_valid_offset = k_length if not CAUSAL else min(k_length, (q_chunk_offset + (qblock_id + 1) * BLOCK_SIZE_Q) * q_pooling_size - q_pooling_size)
    ...
    k_ptrs = k_ptrs + tl.arange(0, BLOCK_SIZE_K)[None, :] * stride_kn + tl.arange(0, HEAD_DIM)[:, None]

    q_ptrs = q + batch_id * stride_qb + head_id * stride_qh + qblock_id * BLOCK_SIZE_Q * stride_qn
    q_ptrs = q_ptrs + tl.arange(0, BLOCK_SIZE_Q)[:, None] * stride_qn + tl.arange(0, HEAD_DIM)[None, :]
    qblock = tl.load(q_ptrs)
    qblock = (qblock * sm_scale).to(qblock.type.element_ty)

    column_count_ptr = column_count + batch_id * stride_ccb + head_id * stride_cch + qblock_id * BLOCK_SIZE_Q
    column_count_ptr = column_count_ptr + tl.arange(0, BLOCK_SIZE_Q)[:, None]
    column_index_ptr = column_index + batch_id * stride_cib.to(tl.int64) + head_id * stride_cih + qblock_id * BLOCK_SIZE_Q * stride_cin
    column_index_ptr = column_index_ptr + tl.arange(0, BLOCK_SIZE_Q)[:, None] * stride_cin + tl.arange(0, BLOCK_SIZE_K)[None, :]
    ...
    for k_offset in tl.range(k_offset_start, k_offset_end, BLOCK_SIZE_K):
        ...
        if CAUSAL:
            qk_mask = qk_mask & (tl.arange(0, BLOCK_SIZE_Q)[:, None] > ((k_offset - offset_at_causal) // q_pooling_size))
...
def find_best_eff_config(
    ...
    fuse_qk_softmax_minp_wo_causal(avg_q, k, 0, gap, causal, q_pooling_size, k_local_size, causal, group_k_block)
...
def find_all_config(
    ...
    """Sweep all (q_pooling_size, k_local_size, group_k_block) combinations and find the best kernel config.
        ...
        for k_local_size in k_local_size_list:
            ...
            for group_k_block in group_k_block_list:
                ...
                best_configs[key] = {
                    ...
                    **{k: v for k, v in best_eff_config.items() if k not in ['kwargs', 'pre_hook', 'ir_override']}
                }
