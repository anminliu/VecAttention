...
from eval.DiTEvalKit.models.hyvideo.inference import replace_hyvideo_flashattention, replace_hyvideo_attention
from eval.DiTEvalKit.models.hyvideo.utils import get_prompt_length
...
if __name__ == "__main__":
    ...
    transformer = HunyuanVideoTransformer3DModel.from_pretrained(
        args.model_id, subfolder="transformer", torch_dtype=torch.bfloat16, revision='refs/pr/18', local_files_only=True
    )
    flow_shift = 7.0
    ...
    if args.threshold_path is not None and os.path.exists(args.threshold_path):
        args.config.threshold = _load_threshold_pt(
            thr_path=args.threshold_path,
        )
        if torch.cuda.is_available():
            args.config.threshold = args.config.threshold.cuda()
        logger.info(f"Loaded per-head threshold matrix from {args.threshold_path}, threshold shape: {args.config.threshold.shape}")
    ...
    elif args.pattern in ["xattn", "vecattention"]:
        replace_hyvideo_attention(
            pipe,
            args.height,
            args.width,
