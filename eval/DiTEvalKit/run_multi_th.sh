#!/usr/bin/env bash
# Usage:
# bash run_pareto.sh <gpuids> [methods] [backends] [sample_num] [output_root] [prompt_ids] [resolution] [infer_step]
# Examples:
# 1) All methods & both backends (hyvideo, wan) on 4 GPUs:
#    bash run_pareto.sh 0,1,2,3
# 2) Specific methods and backend:
#    bash run_pareto.sh 0,1 "dense,xattn,SVG" "hyvideo" 32
# 3) Specific prompt_ids for testing (overrides sample_num):
#    bash run_pareto.sh 0 "dense" "hyvideo" "" "" "10,25,33"
# 4) Custom output root:
#    bash run_pareto.sh 0,1 "" "" 40 "/my/outputs"
#
# Arguments:
# gpuids          required, comma-separated GPU indices
# methods         optional, comma-separated method set, default: dense,xattn,SVG,SAP,vecattention
# backends        optional, comma-separated backend set, default: hyvideo,wan
# sample_num      optional, number of samples (evenly spaced). Ignored if prompt_ids is set
# output_root     optional, override output root; if empty, each backend uses its default
...
run_with_torchrun() {
  ...
  # Log directory placed under output_root/method
  ...
  # Write a separator header before each run
  {
    echo "===== RUN START $(ts) backend=${backend} method=${method} variant=${variant_tag} ====="
  } > "${log_out}"
  {
    echo "===== RUN START $(ts) backend=${backend} method=${method} variant=${variant_tag} ====="
  } > "${log_err}"

  # Pin triton version for SVG/svg
  local uv_extra=()
...
for BACKEND in "${BACKENDS[@]}"; do
  ...
  for METHOD in "${METHODS[@]}"; do
    case "${METHOD}" in
      ...
      SVG|svg)
        for D in 0.4 0.5; do
          run_with_torchrun "${BACKEND}" SVG "density_${D}" _ "${OUTPUT_ROOT}" \
            --density "${D}" \
            --num-sampled-rows 64 \
            --add-logging-file \
     ...
     vecattention_wo_DP|segsp_wo_DP)
        # Only pass threshold directly; this branch does not use any threshold path
        ...
        for TH in 0.0001 0.0005 0.001 0.005 0.01 0.05; do
     ...
     vecattention|segsp)
        ...
        # Read env var and determine whether to enable wan single-file mode
        GUIDANCE_SCALE="${GUIDANCE_SCALE:-}"
        WAN_USE_SINGLE_POS=0
        if [[ -n "${GUIDANCE_SCALE}" ]]; then
          # Float comparison: enable single-file mode if < 1
          if awk "BEGIN{exit !(${GUIDANCE_SCALE} < 1)}"; then
            WAN_USE_SINGLE_POS=1
          fi
        fi

        # Select directories, with env var override support
        POS_DIR="${POS_DIR:-}"
        NEG_DIR="${NEG_DIR:-}"
        ...
        # Iterate over target sparsities, matching filenames by S value only
        for SP in 0.450 0.550 0.650; do
          ...
          if [ "${BACKEND}" = "wan" ] && [[ "${WAN_USE_SINGLE_POS}" -ne 1 ]]; then
            ...
            if [ -z "${POS_FILE}" ] || [ -z "${NEG_FILE}" ]; then
              echo "[WARN] Threshold file not found: backend=${BACKEND} S=${SP} pos=${POS_FILE:-<none>} neg=${NEG_FILE:-<none>} (skip)"
              continue
            fi
            ...
            if [ -z "${POS_FILE}" ]; then
              echo "[WARN] Threshold file not found: backend=${BACKEND} S=${SP} dir=${POS_DIR} (skip)"
              continue
            fi
            run_with_torchrun "${BACKEND}" vecattention "sparsity_${SP}" _ "${OUTPUT_ROOT}" \
              --block-size-q "${BLOCK_SIZE_Q}" \
        ...
        ;;
      all)
        echo "[WARN] 'all' skipped, placeholder — no processing needed"
        ;;
      *)
        echo "[WARN] Unknown method: ${METHOD}, skip"
        ;;
    esac
  done
done
