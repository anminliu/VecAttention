...
from spattn.src.models.utils import SPARSITY_RECORD, customize_prefill_attention
...
class WanAttn_SVGAttn_Processor2_0:
    ...
    def attention_core_logic(
        ...
        if full_attention_flag:
            ...
            if self.fastprefill_config is not None and self.fastprefill_config.output_time:
            ...
            if start_evt is not None and end_evt is not None:
                end_evt.record(stream=cur_stream)
...
class WanAttn_FastPrefill_Processor(WanAttn_SVGAttn_Processor2_0):

    first_layers_fp = 0
    first_times_fp = 0

    fastprefill_config = None
    step_ctx = None
    ...
    def attention_core_logic(
        ...
        if self.fastprefill_config.metric == "vecattention" and self.layer_idx == 0:
            step_ctx = self.step_ctx
            cur_time_step = timestep[0].item()
            change_step = step_ctx.pre_time_step is None or step_ctx.pre_time_step != cur_time_step
            if change_step:
                step_ctx.current_rel_step += 1

            if step_ctx.is_cfg:
                if change_step:
                    # First pass of this diffusion step → positive path
                    self.fastprefill_config.threshold = self.fastprefill_config.pos_threshold
                    logger.info(f"Layer {self.layer_idx} - step {step_ctx.current_rel_step}: "
                                f"timestep {cur_time_step}, using pos_threshold")
                else:
                    # Second pass of this diffusion step → negative path
                    self.fastprefill_config.threshold = self.fastprefill_config.neg_threshold
                    logger.info(f"Layer {self.layer_idx} - step {step_ctx.current_rel_step}: "
                                f"timestep {cur_time_step} unchanged, using neg_threshold")

            step_ctx.pre_time_step = cur_time_step
        ...
        if not full_attention_flag and self.fastprefill_config.dump_qk_dir:
            step_ctx = self.step_ctx
            # Decide whether to start dumping this pass.
            # Positive pass (dump_qk_negative=False): start from layer 0.
            # Negative pass (dump_qk_negative=True): set the flag at layer 39 of the
            #   positive pass so that the following negative-pass call dumps all layers.
            if not step_ctx.dump_qk_negative and not step_ctx.dump_qk_started:
                step_ctx.dump_qk_started = True

            if step_ctx.dump_qk_started:
                dump_dir = self.fastprefill_config.dump_qk_dir
                input_id = self.fastprefill_config.input_id if self.fastprefill_config is not None else "input"
                dir_name = os.path.join(dump_dir, f"{input_id}")
                os.makedirs(dir_name, exist_ok=True)
                if not os.path.exists(os.path.join(dir_name, f"layer_{self.layer_idx}_qk.pt")):
                    torch.save({
                        "query_states": query.detach().cpu(),
                        "key_states": key.detach().cpu(),
                    }, os.path.join(dir_name, f"layer_{self.layer_idx}_qk.pt"))
                    logger.info(f"Dumped QK for layer {self.layer_idx} to {dir_name}")
                if self.layer_idx == 39:
                    # All layers dumped for this pass — mark complete.
                    step_ctx.dump_qk_done = True
            elif self.layer_idx == 39 and step_ctx.dump_qk_negative:
                # End of positive pass: arm the flag so the next (negative) pass dumps.
                step_ctx.dump_qk_started = True
            full_attention_flag = True

        if (self.fastprefill_config.dump_qk_dir
                and self.step_ctx.dump_qk_done
                and self.step_ctx.dump_qk_started):
            sys.exit(0)  # Exit after all layers have been dumped

        if full_attention_flag:
            output_hidden_states = self.flash_attention(query, key, value)
            return output_hidden_states.reshape(cfg, num_heads, seq_len, dim)
        else:
            output_hidden_states = customize_prefill_attention(
                self,
                query,
                key,
                value,
                ...
                attention_mask=None,
                position_ids=None,
                config=self.fastprefill_config
            )
            return output_hidden_states.reshape(cfg, num_heads, seq_len, dim)
