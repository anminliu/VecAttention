...
from ..diffusion_context import DiffusionStepContext
...
def replace_wan_attention(
    ...
    step_ctx: DiffusionStepContext = None,
):
    ...
    if pattern == "SVG":
        ...
        if attention_backend == "flexattn":
            ...
            AttnModule.block_mask = block_mask.to("cuda")
        ...
        for layer_idx, m in enumerate(pipe.transformer.blocks):
            if hasattr(m.attn1, "processor"):  # Check if processor exists
                ...
                m.attn1.set_processor(current_processor)
    elif pattern in ["xattn", "vecattention", "dense"]:

        AttnModule = WanAttn_FastPrefill_Processor
        ...
        AttnModule.step_ctx = step_ctx
        AttnModule.num_key_value_groups = 1

        for layer_idx, m in enumerate(pipe.transformer.blocks):
            self_attn = m.attn1
            self_attn.set_processor(AttnModule(layer_idx=layer_idx))
