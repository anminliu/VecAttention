...
class HunyuanVideoAttnProcessor2_0_FlashAttention:
    ...
    def __call__(
        ...
        if start_evt is not None and end_evt is not None:
            ...
            # Store only the (start_evt, end_evt) pair
...
try:
    ...
    if os.getenv("USE_FAST_KERNEL", "1") == "0":
        print("USE_FAST_KERNEL is set to 0, disable fast kernels.")
        raise ImportError("USE_FAST_KERNEL is set to 0")
    ...
    def attention_core_logic(
        ...
        if full_attention_flag:
            ...
            if self.fastprefill_config is not None and self.fastprefill_config.output_time:
                ...
                start_evt, end_evt, cur_stream = None, None, None
            if start_evt is not None:
                start_evt.record(stream=cur_stream)
...
# ---- Custom Sparse Attn ----
class Hunyuan_FastPrefill_Processor:
    ...
    def __call__(
        ...
        if not full_attention_flag and self.fastprefill_config.dump_qk_dir:
            ...
            if not os.path.exists(os.path.join(dir_name, f"layer_{self.layer_idx}_qk.pt")):
                torch.save({
                    "query_states": query[:, :, : img_len].detach().cpu(),
                    "key_states": key[:, :, : cu_seqlens_kv[1]].detach().cpu(),
        ...
        if full_attention_flag:
            ...
            q1 = query[:, :, : cu_seqlens_q[1]]
            k1 = key[:, :, : cu_seqlens_kv[1]]
            v1 = value[:, :, : cu_seqlens_kv[1]]

            q2 = query[:, :, cu_seqlens_q[1] : cu_seqlens_q[2]]
            k2 = key[:, :, cu_seqlens_kv[1] : cu_seqlens_kv[2]]
            v2 = value[:, :, cu_seqlens_kv[1] : cu_seqlens_kv[2]]

            if self.fastprefill_config.metric == "vecattention":
                q_img = q1[:, :, : img_len]
                q_txt = q1[:, :, img_len:]


                x1_img = customize_prefill_attention(
                    self,
                    q_img,
                    k1,
                    v1,
                ...
                x1 = torch.cat([x1_img, x1_txt], dim=2)
                assert x1.shape[2] == q1.shape[2], f"x1 shape: {x1.shape}, q1 shape: {q1.shape}"
            else:
                x1 = customize_prefill_attention(
                    self,
                    q1,
                    k1,
                    v1,
