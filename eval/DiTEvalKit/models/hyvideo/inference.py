...
def replace_hyvideo_attention(
    ...
    if pattern == "SVG":
        ...
        print(f"Spatial_width: {spatial_width}. Sparsity: {sparsity}")
        ...
        for layer_idx, m in enumerate(pipe.transformer.single_transformer_blocks):
            ...
            # )
    elif pattern in ["xattn", "vecattention"]:

        AttnModule = Hunyuan_FastPrefill_Processor

        AttnModule.first_layers_fp = first_layers_fp
        AttnModule.first_times_fp = first_times_fp
        AttnModule.logging_file = logging_file

        AttnModule.fastprefill_config = config
        AttnModule.num_key_value_groups = 1
        ...
        for layer_idx, m in enumerate(pipe.transformer.transformer_blocks):
            self_attn = m.attn
            self_attn.processor = AttnModule(layer_idx=layer_idx)
            # print(f"Replaced {pattern} block for Double Stream Transformer at layer {layer_idx}")

        for layer_idx, m in enumerate(pipe.transformer.single_transformer_blocks):
            self_attn = m.attn
            self_attn.processor = AttnModule(layer_idx=layer_idx + len(pipe.transformer.transformer_blocks))
            # print(
            #     f"Replaced {pattern} block for Single Stream Transformer at layer {layer_idx + len(pipe.transformer.transformer_blocks)}"
            # )

    else:
        raise NotImplementedError(f"Invalid pattern: {pattern}")
