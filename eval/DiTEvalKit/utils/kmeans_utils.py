...
try:
    from cuvs.cluster.kmeans import KMeansParams, fit as _cuvs_fit
...
def kmeans_rapidai(
    ...
    if not _CUVS_AVAILABLE:
        raise ImportError(
            "cuvs-cu12 is required for SAP (kmeans_rapidai). "
            "Install it with: uv pip install cuvs-cu12 --extra-index-url=https://pypi.nvidia.com"
        )
    ...
    new_centroids, inertia, n_iter_ = _cuvs_fit(params, tensor, current_centroids)  # Added handle=None
